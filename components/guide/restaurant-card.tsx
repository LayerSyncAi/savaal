...
export function RestaurantCard({ restaurant }: RestaurantCardProps) {
	...
	const featuredScores = featuredCategoryOrder
		...
		.filter((score): score is RestaurantInfo["scores"][number] =>
			Boolean(score)
		);
	...
	return (
		...
		>
			<div
				className={`relative h-full min-h-[420px] w-full transition-transform duration-700 [transform-style:preserve-3d] ${
					isFlipped
			...
			>
				<article
					className={`absolute inset-0 flex h-full w-full flex-col overflow-hidden rounded-2xl border border-orange-100/70 bg-neutral-900 shadow-md transition-opacity duration-500 [backface-visibility:hidden] ${
				...
				>
					...
					<div className="relative flex flex-1 flex-col gap-4 p-4 text-white">
						...
						>
							...
							<FaArrowRight className="relative z-10 w-4 h-4 text-orange-500 transition-all duration-300 group-hover/start:translate-x-0.5 group-hover/start:scale-110" />
						</Link>
					</div>
				</article>

				<article
					className={`absolute inset-0 flex h-full w-full flex-col rounded-2xl border border-orange-100 bg-white px-4 py-6 shadow-md transition-opacity duration-500 [backface-visibility:hidden] [transform:rotateY(180deg)] ${
