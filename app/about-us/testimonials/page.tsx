...
import { scrollingTestimonials, type Testimonial } from "@/content/testimonials";
...
export default function AboutUsTestimonialsPage() {
  const stackedTestimonials = useMemo(() => {
    const icons = [SiNike, SiAtlassian, SiDribbble, SiGrubhub, SiKaggle, SiSlack];
      ...
      {Array.from(Array(numTracks).keys()).map((n) => {
        return (
          <button
            ...
            className="relative h-1.5 w-full bg-slate-700"
          >
            {selected === n ? (
              <motion.span
